...
\begin{align*}
    p_{\Theta\\|x}(\theta)
    &\propto \mathcal{N}(x; \theta, \sigma^2)\text{Exp}(\theta; \lambda) \\
    &\propto \exp \left[
        - \dfrac{1}{2\sigma^2} \left(
            x - \sigma^2 \lambda - \theta
        \right)^2
...
Nếu quan tâm, mọi người có thể thử tại [đây](https://www.desmos.com/calculator/jmscgmo8iy) để so sánh giữa phân phối hậu nghiệm chính xác và phân phối biến phân tối ưu.

<div align="center">
    <iframe src="https://www.desmos.com/calculator/jmscgmo8iy?embed" width="500" height="500" style="border: 1px solid #ccc" frameborder=0></iframe>
</div>

Trong hình trên, $$\lambda = 3, \sigma^2 = 0.25, x = 1.5$$, đường xanh lá tương ứng với phân phối hậu nghiệm chính xác, đường đỏ tương ứng với phân phối biến phân tối ưu.
...
$$
    q_{\Theta}(\theta; \phi) = \text{Exp}(\theta; \phi) = \phi \exp(-\phi \theta) \mathbb{1}_{[0, \infty)}(\theta)
$$
