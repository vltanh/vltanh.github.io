...
$$
    \begin{align*}
        ...
        \text{Exp}(\theta; \lambda) &= \lambda \exp(-\lambda \theta) \mathbb{I}_{\theta \geq 0}
    \end{align*}
$$
...
$$
\begin{align*}
    &\int_{-\infty}^{\infty} p_{X, \Theta}(x, \theta) d\theta \\
    =& \int_{-\infty}^{\infty} \mathcal{N}(x; \theta, \sigma^2)\text{Exp}(\theta; \lambda) d\theta \\
...
$$
    \phi^* = \arg\max_{\phi} \text{ELBO}_{p_{X, \Theta}}(q_{\Theta}(\theta; \phi))
$$
