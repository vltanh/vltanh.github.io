...
(function () {
  ...
  function runKernel(g) {
    ...
    return L.optimisePartition(G, L.CPM(g), seed, {
      recordTrace: true,
  ...
  const singletonMem = F.nodes.map(function (_, i) {
    return i;
  });
  ...
  const refineWalker = P.mountRefineWalker({
    ...
    ctlPrefix: "g-refine",
    events: refineEvents,
    initialMembership: singletonMem,
  ...
  function rebuildSnapshots() {
    ...
    // Stage 7: refinement-init snapshot. Fill shows the refined singleton;
    // the outer ring preserves the move-phase community constraint.
    clearHost("g-refine-init-cy");
    P.renderFixture("g-refine-init-cy", {
      membership: singletonMem,
      secondaryMembership: postMove,
      pinned: true,
    });
