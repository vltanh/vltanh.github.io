...
(function () {
  ...
  // tied cuts. Default seed=0 matches the one-time setSeed(0) in main.cpp;
  // nonzero URL seeds are teaching experiments because pipeline.sh does not
  // forward its --seed metadata field to the constrained binary.
  ...
  if (criterionSelect) {
    ...
    for (let i = 0; i < criterionSelect.options.length; i++) {
      if (criterionSelect.options[i].value === criterionInput) {
        criterionSelect.selectedIndex = i;
        matched = true;
        break;
      }
    }
  ...
  function phaseStripHTML(verdict) {
    return (
      ...
      PHASES.map(function (ph) {
        ...
        return (
          '<div class="' + cls + '">' + '<span class="ph-num">step ' + ph.n + "</span>" + '<span class="ph-label">' + ph.label + "</span>" + "</div>"
        );
      }).join("") +
      "</div>"
    );
  ...
  function gaugeHTML(cut, thr) {
    ...
    return (
      '<div class="wcc-gauge">' +
      '<div class="gauge-cap">cut <b>' +
      cut +
      "</b> vs threshold <b>" +
      thr.toFixed(3) +
      "</b>" +
      (Math.abs(thr - cut) <= 1e-9 ? " &middot; within 1e-9 epsilon" : "") +
      "</div>" +
      '<div class="gauge-bar">' +
      '  <div class="gauge-thr"></div>' +
      '  <div class="gauge-marker" style="left:' +
      pct +
      '%"></div>' +
      "</div>" +
  ...
  function thresholdBoxHTML(parsed, n, thr) {
    ...
    if (parsed.kind === "exponential") {
      return '<div class="step-tip">threshold = ' + parsed.c + " &middot; " + n + "<sup>" + parsed.x + "</sup> = " + thr.toFixed(4) + "</div>";
    }
  ...
  function renderCC(result) {
    ...
    result.cc.events.forEach(function (ev) {
      ...
      html +=
        '<div class="wcc-cc-card">' +
        '<div class="cc-id">cluster <b>' +
        ev.clusterIn +
        "</b></div>" +
        '<div class="cc-meta">' +
        ev.nodes.length +
      ...
      ev.components.forEach(function (comp, i) {
        html += '<span class="cc-comp' + (split ? " split" : "") + '">' + "c" + i + " &middot; n=" + comp.length + "</span>";
      });
      html += "</div></div>";
    });
  ...
  function deriveQueueState(initialQueue, events, popIdxBeforeNext) {
    ...
    const queue = initialQueue.map(function (c) {
      return c.slice();
    });
    const survivors = [];
  ...
  function chunkColor(chunk) {
    ...
    const m = chunk.reduce(function (a, b) {
      return Math.min(a, b);
    }, chunk[0]);
  ...
  function chipHTML(chunk, opts) {
    ...
    const cls = "wcc-chip" + (opts.head ? " head" : "") + (opts.kept ? " kept" : "") + (opts.fresh ? " fresh" : "");
    return (
      '<span class="' +
      cls +
      '" title="' +
  ...
  function renderQueuePanel(qstate, currentEv) {
    ...
    if (queueChips) {
      if (qstate.queue.length === 0) {
        ...
        queueChips.innerHTML = qstate.queue
          .map(function (c, i) {
    ...
    if (survChips) {
      if (qstate.survivors.length === 0) {
        ...
        survChips.innerHTML = qstate.survivors
          .map(function (c, i) {
  ...
  function renderRecordTable(events, parsed, currentIdx) {
    ...
    events.forEach(function (ev, i) {
      const cls = i === currentIdx - 1 ? "cur" : i < currentIdx - 1 ? (ev.wellConnected ? "kept" : "push") : "future";
      html +=
        '<tr class="' +
        cls +
        '">' +
        ...
        "</td>" +
        '<td class="num">' +
        ev.threshold.toFixed(3) +
        "</td>" +
        "<td>" +
        (ev.wellConnected ? "KEEP" : "PUSH") +
        "</td>" +
        '<td class="num">' +
        (ev.wellConnected ? "0" : ev.pushedBack.length) +
        "</td>" +
        "</tr>";
    });
  ...
  const TIP_KEEP =
    "Cut value c clears threshold t = pre_log &middot; log(n), strict and outside the 1e-9 epsilon (constrained.h:430-432). Cluster moves to the survivor list in FIFO pop order (constrained.cpp:135-152).";
  const TIP_PUSH =
    "Cut value c does not clear threshold t (within or below). Split at the cut: take connected components on each side, push every size&gt;1 component back onto the queue. In-side components first, then out-side (mincut_only.h:97-122).";
  const TIP_DISCONNECTED =
    "Mincut returned 0 — the induced subgraph is disconnected. Threshold trivially loses. Both sides become their own queue entries.";
  const TIP_EPSILON =
    "cut equals threshold within 1e-9. constrained.h:431's <code>is_close</code> guard treats this as not well-connected: split rather than keep. Triggers on n=10, 100, 1000 with <code>1log_10(n)</code>.";
  ...
  function panelHTML(idx, ev, parsed) {
    ...
    html += '<span class="wcc-verdict ' + verdict + '">' + (ev.wellConnected ? "keep" : "split + push back") + "</span>";
    html +=
      "<span style=\"font-family:'Special Elite',monospace;font-size:.78rem;color:var(--paper-2)\">" +
      "cluster size <b>" +
      ev.clusterSize +
      "</b></span>";
    html += "</div>";
  ...
  function runAndMount(criterionSpec) {
    ...
    result.carve.events.forEach(function (rt) {
      // Current runWCC returns the recursive carve records directly. Keep
      // accepting the older per-component {events:[...]} envelope so saved
      // fixtures and teaching builds remain replayable.
      const records = Array.isArray(rt.events) ? rt.events : [rt];
    ...
    result.cc.events.forEach(function (ev) {
      ev.components.forEach(function (comp) {
        if (comp.length > 1) initialQueue.push(comp.slice());
      });
    });

    // Walker membership snapshots: track surviving partition + queued
    // chunks. snapshot[k] reflects state after event k-1.
    const baseMem = F.nodes.map(function () {
      return -1;
    });
    const chunks = initialQueue.map(function (c) {
      return c.slice();
    });
    const chunkOf = {};
    ...
    const chunkSurvived = chunks.map(function () {
      return false;
    });
    function renderMem() {
    ...
    flat.forEach(function (ev) {
      const sortedEv = ev.cluster.slice().sort(function (a, b) {
        return a - b;
      });
      ...
      for (let k = 0; k < chunks.length; k++) {
        ...
        const sortedCh = chunks[k].slice().sort(function (a, b) {
          return a - b;
        });
        if (sortedCh.join(",") === targetKey) {
          ci = k;
          break;
        }
      }
    ...
    function statusFor(idx, ev) {
      ...
      return (
        "pop " + idx + " &middot; n=" + ev.clusterSize + " &middot; cut=" + ev.cut + " &middot; thr=" + ev.threshold.toFixed(3) + " &middot; " + wc
      );
    }
    ...
    lastController = P.mountStepWalker({
      ...
      sidePanelHTML: function (idx, ev) {
        return panelHTML(idx, ev, parsed);
      },
    ...
    P.renderFixture("g-final-out", {
      membership: Array.from(result.finalAssign),
      pinned: true,
      nodeR: 9,
    });
    ...
    if (summary) {
      ...
      summary.innerHTML =
        "survivors: " + result.numClusters + " &middot; nodes kept: " + kept + " / dropped: " + dropped + " &middot; criterion: " + result.criterion;
    }
  }
