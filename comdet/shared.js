...
(function (global) {
  ...
  const EdgePaths = (function () {
    ...
    function makeSelfLoop(p, r0, opts) {
      ...
      const cxL = p.x - r * LOOP_OFFX,
        cyL = p.y - r * LOOP_OFFY;
      const cxR = p.x + r * LOOP_OFFX,
        cyR = p.y - r * LOOP_OFFY;
      return "M" + sx + "," + sy + " Q" + cxL + "," + cyL + " " + apex.x + "," + apex.y + " Q" + cxR + "," + cyR + " " + ex + "," + ey;
    }
  ...
  const BridgeAnim = (function () {
    function lengthOf(node) {
      return (node.getTotalLength && node.getTotalLength()) || 100;
    }
    function grow(pathSel, opts) {
      ...
      pathSel.each(function () {
        ...
        if (isLoop) {
          sel.attr("stroke-dasharray", len + " " + len).attr("stroke-dashoffset", len);
          ...
          const halfLen = len / 2;
          sel.attr("stroke-dashoffset", 0).attr("stroke-dasharray", "0 " + len + " 0 0");
          const t = sel.transition(tName).duration(duration);
          if (ease) t.ease(ease);
          if (opts.delay) t.delay(opts.delay);
    ...
    function retract(pathSel, opts) {
      ...
      pathSel.each(function () {
        ...
        const len = lengthOf(this);
        if (isLoop) {
          sel.attr("stroke-dasharray", len + " " + len).attr("stroke-dashoffset", 0);
          const t = sel.transition(tName).duration(duration);
          if (ease) t.ease(ease);
          t.attr("stroke-dashoffset", len);
        } else {
          const halfLen = len / 2;
          sel.attr("stroke-dashoffset", 0).attr("stroke-dasharray", halfLen + " 0 " + halfLen + " 0");
          const t = sel.transition(tName).duration(duration);
          if (ease) t.ease(ease);
          t.attrTween("stroke-dasharray", function () {
            return function (k) {
    ...
    function colorize(pathSel, opts) {
      ...
      if (ease) t.ease(ease);
      t.attr("stroke", finalColor).attr("stroke-width", finalWidth).attr("stroke-dasharray", null);
      return t;
    }
  ...
  const VIZ = {
    init(containerId, opts = {}) {
      ...
      const fitOpts = {
        ...
        includeIds: opts.includeOutliers === false ? NODES.filter((n) => CLUSTER_OF[n] !== "OUT") : NODES,
        pad: opts.pad,
      ...
      const nodesData = NODES.filter((n) => includeOutliers || CLUSTER_OF[n] !== "OUT")
        .filter((n) => posSrc[n] != null)
        .map((id) => {
          const hx = posSrc[id].x,
            hy = posSrc[id].y;
          return {
            id: String(id),
      ...
      let onNodeTap = null,
        onEdgeTap = null;
      let onNodeEnter = null,
        onNodeLeave = null;
      let onEdgeEnter = null,
        onEdgeLeave = null;
      ...
      function draw() {
        ...
        if (useAnim) {
          const xd = nextExitDelay,
            xs = nextExitStagger,
            xFromIdFallback = nextExitFromId;
          ...
          linkExit.each(function (d, i) {
            ...
            if (fromSrc === null) {
              ...
              tr.attrTween("d", function () {
                const sx0 = d.source.x,
                  sy0 = d.source.y;
                const ex0 = d.target.x,
                  ey0 = d.target.y;
        ...
        const linkEnter = linkSel
          ...
          .on("click", function (ev, d) {
            if (onEdgeTap) onEdgeTap(d, ev);
          })
          .on("mouseenter", function (ev, d) {
            if (onEdgeEnter) onEdgeEnter(d, ev);
          })
          .on("mouseleave", function (ev, d) {
            if (onEdgeLeave) onEdgeLeave(d, ev);
          });
        if (useAnim) {
          linkEnter.style("opacity", 0);
          const ed = nextEnterDelay,
            es = nextEnterStagger,
            fromIdFallback = nextEnterFromId;
          ...
          enterRunners.push(() => {
            linkEnter.each(function (d, i) {
              ...
              const sel = d3.select(node);
              const tr = sel
                .transition("vizLinkEnter")
                .delay(ed + es * i)
                .duration(eDur)
                .ease(d3.easeCubicOut);
              if (fromSrc === null) {
                tr.style("opacity", 1).on("end", function () {
        ...
        if (useAnim) {
          groupExit.style("pointer-events", "none").transition("vizGroupExit").duration(animDur).style("opacity", 0).remove();
        } else {
          groupExit.remove();
        }
        ...
        const groupEnter = groupSel.enter().append(
          useAnim
            ? function () {
        ...
        groupEnter
          ...
          .on("click", function (ev, d) {
            if (onNodeTap) onNodeTap(d, ev);
          })
          .on("mouseenter", function (ev, d) {
            if (onNodeEnter) onNodeEnter(d, ev);
          })
          .on("mouseleave", function (ev, d) {
            if (onNodeLeave) onNodeLeave(d, ev);
          });
        if (showLabels) {
          groupEnter
            .append("text")
        ...
        const circles = groups.select("circle.viz-node").attr("class", (d) => classStr("viz-node", d.cls));
        ...
        if (useAnim) {
          ...
          enterRunners.push(() => {
            groupEnter.each(function () {
              this.classList.remove("viz-hidden");
            });
      ...
      function clampedBoundaryEdge(sx, sy, ex, ey, r1, r2) {
        ...
        const off1 = r1 * k,
          off2 = r2 * k;
        const psx = sx + ux * off1,
          psy = sy + uy * off1;
        const pex = ex - ux * off2,
          pey = ey - uy * off2;
      ...
      const handle = {
        ...
        addEdges(edges, edgeOpts) {
          ...
          (edges || []).forEach((e, i) => {
            const l = normEdge(e, links.length + i);
            if (l) links.push(l);
          });
          draw();
          nextEnterDelay = 0;
        ...
        removeEdges(pred) {
          ...
          links.length = 0;
          keep.forEach((l) => links.push(l));
          draw();
        },
        clearEdges() {
          handle.removeEdges(null);
        },
        hasEdge(id) {
        ...
        toggleEdgeClass(id, cls, on) {
          on ? handle.addEdgeClass(id, cls) : handle.removeEdgeClass(id, cls);
        },
        addAllEdgeClass(cls) {
          links.forEach((e) => (e.cls = (e.cls + " " + cls).trim()));
          draw();
        },
        clearAllEdgeClass(cls) {
          links.forEach(
            (e) =>
              (e.cls = e.cls
                .split(/\s+/)
        ...
        animateEdgeOpacityByClass(cls, target, durMs = 300) {
          gLinks
            .selectAll("path.viz-edge")
            .filter(function (d) {
              return (" " + d.cls + " ").includes(" " + cls + " ");
            })
            .transition()
            .duration(durMs)
  ...
  // Back-compat alias: old pages that still reference COMDET.CY.init
  ...
  function makeTooltip(handle, container) {
    ...
    handle.onNodeHoverLeave(function () {
    ...
    handle.onEdgeHoverEnter(function (d, ev) {
      ...
      tip.innerHTML = '<div class="hd">edge</div>' + "<div><b>" + src + "</b>, <b>" + tgt + "</b></div>" + '<div class="dim">' + kind + "</div>";
      tip.classList.add("on");
      place(ev.clientX, ev.clientY);
    });
    handle.onEdgeHoverLeave(function () {
      tip.classList.remove("on");
    });
    return tip;
  }
  ...
  function stepController(opts) {
    ...
    randStepBtn &&
      randStepBtn.addEventListener("click", () => {
        if (isLocked()) return;
        // onRandStep return values:
        //   COMDET.SKIP  — handler ran its own render (e.g. drove an
  ...
  //   viz          optional COMDET.VIZ handle. When provided, the helper
  ...
  function mountGxPanel(opts) {
    ...
    wrap.innerHTML =
      '<button id="' +
      opts.prefix +
      '-toggle" class="gx-panel-toggle" type="button">' +
      (opts.title || "") +
      ' <span id="' +
      opts.prefix +
      '-arrow">&#9662;</span></button>' +
  ...
  //   edges         optional array of {u,v} edges. Default: COMDET.EDGES
  //                 in faint grey.
  //
  ...
  function singletonOpener(opts) {
    ...
    if (!host) return null;
    host.innerHTML = "";
    const svg = d3.select(host).append("svg").attr("class", "viz-svg").attr("viewBox", fitViewBoxAttr()).attr("preserveAspectRatio", "xMidYMid meet");
    const colourOf = {};
    opts.clusters.forEach((cd) =>
      cd.nodes.forEach((n) => {
        colourOf[n] = cd.color;
      })
    );
    ...
    function syncRects() {
      rects.forEach((r) => {
        ...
        let w = MX - mx + 2 * padX,
          h = MY - my + 2 * padY;
        const labelW = r.label.node().getComputedTextLength ? r.label.node().getComputedTextLength() : 0;
  ...
  // whose match-edges are owned by COMDET.spokeLayer. Pass the snap flag
  ...
  function diffPlacedById(beforeList, afterList, placedEntry) {
    ...
    const beforeById = new Map(
      (beforeList || []).map(function (e) {
        return [e.id, e];
      })
    );
    const afterById = new Map(
      (afterList || []).map(function (e) {
        return [e.id, e];
      })
    );
    const removes = [],
  ...
  function walkerMarkJust(pairs, idx, opts) {
    ...
    for (let i = 0; i < idx - 1; i++) {
      if (keyOf(pairs[i].u, pairs[i].v) === k) {
        parallel = true;
        break;
      }
    }
  ...
  function wireWalker(opts) {
    const { prefix, total, onRender, onRandStep, onRandAll, getLocked, keyboard, randAtStart, curId, totId, randStepDisabledAt } = opts;
  ...
  function rerollWalker(opts) {
    ...
    let ctl = null;
    const wrappedRandStep = onRandStep
      ? (idx) => {
    ...
    const wrappedRandAll = onRandAll
      ? (idx) => {
  ...
  function makeRewireRender(opts) {
    ...
    function render(step, snap) {
      ...
      if (!adjacent || snap) {
        ...
        const crossesDropStale = isDropStale && (lastRendered === null || getLastStep() <= opsAppliedStep);
        if (crossesDropStale) {
      ...
      const removes = beforeList
        .filter(function (e) {
          return !afterIds.has(e.id);
        })
        .map(placedEntry);
      const adds = afterList
        .filter(function (e) {
          return !beforeIds.has(e.id);
        })
        .map(placedEntry);
    ...
    render.setLastRendered = function (list) {
      lastRendered = list;
    };
    return render;
  }
  ...
  //   const backdrop = COMDET.clusterPostBackdrop(R, {
  //     idPrefix: "bp-cluster-",
  ...
  // so referencing COMDET.spokeLayer at module-eval time is undefined.
  // Defer resolution to call time when both modules are loaded.
  function spokeLayerHandle() {
    return (typeof window !== "undefined" ? window : global).COMDET.spokeLayer;
  }
  ...
  function rerollClusterRewireRange(opts) {
    ...
    if (opts.storeShape === "flat") {
      ...
      baseOps.forEach(function (o) {
        if (!visited.has(o.cluster)) {
          visited.add(o.cluster);
          visitOrder.push(o.cluster);
        }
      });
      const flat = [];
      visitOrder.forEach(function (c) {
        (next[c] || []).forEach(function (o) {
          flat.push(o);
        });
      });
      return flat;
  ...
  function replayFinalSwap(opts) {
    ...
    for (let i = 0; i < cap; i++) {
      ...
      out.push({
        p1: op.p1.slice(),
        p2: op.p2.slice(),
        newp1: op.newp1.slice(),
        newp2: op.newp2.slice(),
        coin: op.coin,
        keep1: op.keep1,
        keep2: op.keep2,
      });
    ...
    while (recycle.length > 0) {
      ...
      out.push({ p1: p1.slice(), p2: p2.slice(), newp1: newp1.slice(), newp2: newp2.slice(), coin, keep1, keep2 });
    }
    return out;
  }
  ...
  function linksRow(opts = {}) {
    ...
    return (
      ...
      '<a href="' +
      NOTES +
      '" target="_blank" rel="noopener" aria-label="algorithm notes">' +
  ...
  function injectBackToTop() {
    ...
    btt.innerHTML =
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 20 Q 11 13 12 5"/><path d="M7 10 Q 9 8 12 5 Q 15 8 17 10"/></svg>';
  ...
  //
  // opts:
  //   positions: id → {x, y}. Default: COMDET.POSITIONS.
  //   includeIds: array of ids to wrap. Default: COMDET.NODES.
  ...
  function bindClusterRowHover(gBars, viz, opts) {
    ...
    if (nameOfNode) {
      viz.onNodeHoverEnter(function (d) {
        focusOn(nameOfNode(parseInt(d.id, 10)));
      });
      viz.onNodeHoverLeave(function () {
        focusClear();
      });
  ...
  // ── Concept-first lesson guide ──────────────────────────────────
  // The algorithm pages are deliberately deep implementation traces. This
  // summary layer gives a reader the mental model before they enter the long
  // stage sequence. Copy is kept alongside the shared UI so all pages use the
  // same vocabulary and the same source-of-truth distinctions.
  const LESSONS = {
    "leiden-cpm": {
      kind: "detector · objective optimisation",
      community: "a group whose internal-edge reward exceeds CPM's size penalty",
      flow: ["Score groups with CPM", "Move, then refine nodes", "Collapse connected pieces and repeat"],
      use: "Use when you want a strong general-purpose partition and an explicit resolution control.",
      caution: "The resolution γ is substantive: low values merge across weak bridges; high values create small groups and singletons.",
    },
    "leiden-mod": {
      kind: "detector · objective optimisation",
      community: "a group with more internal edge weight than a degree-preserving null model predicts",
      flow: ["Score groups with modularity", "Move, then refine nodes", "Collapse connected pieces and repeat"],
      use: "Use for a parameter-free modularity baseline with Leiden's connectivity-aware refinement.",
      caution: "The optimiser is improved, but modularity still has a resolution limit and can absorb small real communities.",
    },
    louvain: {
      kind: "detector · objective optimisation",
      community: "a local peak of Newman–Girvan modularity",
      flow: ["Start with singleton groups", "Greedily move nodes", "Collapse groups and repeat"],
      use: "Use as a fast, widely recognised modularity baseline.",
      caution: "Louvain does not refine a group before collapse, so an output community can be badly connected or disconnected.",
    },
    infomap: {
      kind: "detector · information theory",
      community: "a region where a random walker reuses a short local codebook before exiting",
      flow: ["Estimate visit flow", "Shorten the map equation", "Tune coarse and fine modules"],
      use: "Use when direction, weight, or flow persistence is the community signal you care about.",
      caution: "Teleportation and the two-level codebook affect small or weakly visited modules; this is not a density objective.",
    },
    ikc: {
      kind: "detector · dense-core peeling",
      community: "a connected piece of a high-k core that survives the output gates",
      flow: ["Compute core numbers", "Extract the highest surviving core", "Remove it and repeat"],
      use: "Use when dense, cohesive cores matter more than assigning every node.",
      caution: "Sparse but meaningful groups may be left unassigned, and the shipped modularity gate is effectively disabled.",
    },
    "sbm-flat-best": {
      kind: "selector · Bayesian block models",
      community: "the block assignment that gives the shortest complete description of graph plus model",
      flow: ["Fit DC, NDC, and planted models", "Compare description lengths", "Return the lowest-cost model"],
      use: "Use when you want the data to choose among flat SBM assumptions.",
      caution:
        "Model scores are comparable only when their coding conventions match; the search is stochastic and more expensive than greedy methods.",
    },
    "sbm-flat-dc": {
      kind: "detector · Bayesian block model",
      community: "a block with a characteristic connection pattern after conditioning on node degrees",
      flow: ["Encode degrees separately", "Propose one-node block moves", "Keep moves favoured by description length"],
      use: "Use for networks with broad degree variation where hubs should not become communities by themselves.",
      caution: "Degree correction can remove degree variation that is genuinely part of the community signal.",
    },
    "sbm-flat-ndc": {
      kind: "detector · Bayesian block model",
      community: "a block whose members share both connectivity and degree pattern under one block rate",
      flow: ["Encode block edge counts", "Propose one-node block moves", "Keep moves favoured by description length"],
      use: "Use when within-block degree homogeneity is plausible or degree itself carries group information.",
      caution: "On heavy-tailed networks it can isolate hubs into degree-based blocks rather than structural communities.",
    },
    "sbm-flat-pp": {
      kind: "detector · assortative block model",
      community: "one of several groups sharing a common within-group rate above a common between-group rate",
      flow: ["Assume two edge rates", "Sample candidate partitions", "Keep the lowest description length"],
      use: "Use when groups are expected to be uniformly assortative and roughly comparable in structure.",
      caution: "The shared-rate assumption is restrictive when some communities are dense and others sparse or disassortative.",
    },
    "sbm-nested-best": {
      kind: "selector · hierarchical block models",
      community: "a lowest-level block embedded in the hierarchy with the shortest total description",
      flow: ["Fit nested DC and NDC", "Rebuild block graphs by level", "Return the lower total description length"],
      use: "Use when communities exist at several scales and you want the degree assumption selected from data.",
      caution: "The hierarchy costs extra model description and the live page currently exposes less of the upper-level sampler than level 0.",
    },
    "sbm-nested-dc": {
      kind: "detector · hierarchical block model",
      community: "a degree-corrected block at level 0, grouped again by coarser block interaction",
      flow: ["Fit degree-corrected base blocks", "Collapse to a block graph", "Repeat the model at higher levels"],
      use: "Use for multiscale structure in networks with heterogeneous node degrees.",
      caution: "The hierarchy and degree prior increase model complexity; the live walker emphasises level 0.",
    },
    "sbm-nested-ndc": {
      kind: "detector · hierarchical block model",
      community: "a degree-homogeneous base block embedded in coarser block groupings",
      flow: ["Fit non-degree-corrected base blocks", "Collapse to a block graph", "Repeat the model at higher levels"],
      use: "Use for multiscale structure when degrees are narrow within blocks or degree is meaningful signal.",
      caution: "Hub-driven blocks can appear at level 0, and the live walker emphasises that level rather than every upper sweep.",
    },
    cc: {
      kind: "post-processor · connectivity repair",
      community: "one connected component inside an existing cluster label",
      flow: ["Remove cross-label edges", "Run BFS inside each label", "Split components and drop singletons"],
      use: "Use as a cheap safety pass when a base detector may emit disconnected labels.",
      caution: "Connected is a weak guarantee: a single bridge edge is enough for a cluster to pass unchanged.",
    },
    wcc: {
      kind: "post-processor · mincut repair",
      community: "a connected piece whose minimum cut strictly exceeds a size-scaled threshold",
      flow: ["Seed connected components", "Compare mincut with f(n)", "Split failures and recurse"],
      use: "Use when merely connected is too weak and you want a tunable bottleneck guarantee without re-clustering.",
      caution: "The threshold changes coverage, and singleton cut sides are pruned rather than retained as communities.",
    },
    cm: {
      kind: "post-processor · cut and re-cluster",
      community: "a well-connected leaf produced after repeated mincut tests and base-method replays",
      flow: ["Test a candidate's mincut", "Re-cluster each failing side", "Track children and repeat"],
      use: "Use when weak clusters should be reconsidered by the base detector, not only split along one cut.",
      caution: "It is the most expensive repair and wrapper-level size filters can remove leaves the binary itself kept.",
    },
    viecut: {
      kind: "primitive · global minimum cut",
      community: "not applicable—VieCut finds graph bottlenecks used by WCC and CM",
      flow: ["Contract provably safe edges", "Build a cactus of all mincuts", "Choose a balanced cut"],
      use: "Use as the cut engine when downstream logic needs an exact global mincut and a useful bipartition.",
      caution: "VieCut is not a standalone community detector; its output only becomes a clustering through recursive post-processing.",
    },
  };

  function lessonPageKey() {
    const file = (global.location.pathname.split("/").pop() || "").replace(/\.html$/, "");
    return file || "index";
  }

  function mountLessonGuide() {
    const meta = LESSONS[lessonPageKey()];
    const main = document.querySelector("main.page");
    if (!meta || !main || main.querySelector(".lesson-guide")) return;
    const stages = Array.from(main.querySelectorAll("section.stage"));
    stages.forEach(function (stage, i) {
      if (!stage.id) stage.id = "lesson-stage-" + (i + 1);
    });
    function stageLink(pattern, fallback) {
      const found = stages.find(function (stage) {
        return pattern.test(stage.textContent.toLowerCase());
      });
      return found || stages[fallback] || null;
    }
    const walker = stageLink(/walker|live trace|step through|interactive/, 0);
    const result = stageLink(/final partition|final output|output next|convergence/, Math.max(0, stages.length - 2));
    const guide = document.createElement("section");
    guide.className = "lesson-guide";
    guide.setAttribute("aria-labelledby", "lesson-guide-title");
    const toc = stages
      .map(function (stage, i) {
        const h = stage.querySelector("h2");
        const label = h ? h.textContent.trim() : "Section " + (i + 1);
        return '<li><a href="#' + stage.id + '"><span>' + String(i + 1).padStart(2, "0") + "</span>" + label + "</a></li>";
      })
      .join("");
    guide.innerHTML =
      '<div class="lesson-eyebrow">algorithm at a glance · start with the CS bridge, then walk the state</div>' +
      '<div class="lesson-head"><div><h2 id="lesson-guide-title">Community means <em>' +
      meta.community +
      '</em>.</h2><span class="lesson-kind">' +
      meta.kind +
      "</span></div>" +
      '<div class="lesson-actions">' +
      (walker ? '<a class="lesson-action primary" href="#' + walker.id + '">open the walker</a>' : "") +
      (result ? '<a class="lesson-action" href="#' + result.id + '">see the result</a>' : "") +
      "</div></div>" +
      '<ol class="lesson-flow">' +
      meta.flow
        .map(function (step, i) {
          return "<li><span>" + (i + 1) + "</span><b>" + step + "</b></li>";
        })
        .join("") +
      "</ol>" +
      '<div class="lesson-judgement"><p><strong>Use it when</strong>' +
      meta.use +
      "</p>" +
      "<p><strong>Watch out</strong>" +
      meta.caution +
      "</p></div>" +
      '<details class="lesson-toc"><summary>Browse all ' +
      stages.length +
      " teaching sections</summary><ol>" +
      toc +
      "</ol></details>";
    const anchor = main.querySelector(".hook, .algo-meta, section.stage");
    if (anchor) main.insertBefore(guide, anchor);
    ...
    function updateProgress() {
      ...
      bar.style.transform = "scaleX(" + Math.min(1, Math.max(0, global.scrollY / max)) + ")";
      scheduled = false;
    }
    global.addEventListener(
      ...
      function () {
        if (!scheduled) {
          scheduled = true;
          global.requestAnimationFrame(updateProgress);
        }
      },
      { passive: true }
    );
  ...
  global.COMDET = {
    ...
    rerollWalker,
    makePageBus,
    makeRewireStateAtStep,
    makeRewireRender,
    clusterPostBackdrop,
    followCursor,
    clusterPrePairsByName,
    globalPrePairsTuples,
    clusterEdgeEkeys,
    rerollClusterRewireRange,
    replayFinalSwap,
    abcdClusterRewireApplyOp,
    abcdBgRewireApplyOp,
    snapOrSync,
    diffPlacedById,
    playManyDiff,
    singletonOpener,
    defaultSingletonClusters,
    bindPanelToggle,
    mountGxPanel,
    walkerMarkPlaced,
    walkerMarkJust,
  ...
  // Keep the evidence-heavy paper → production → walker audit separate from
  // the general UI helpers, but load it on every comdet route. The script is
  // intentionally data-driven so coverage can be checked for all 16 pages.
  const auditScript = document.createElement("script");
  auditScript.src = "./js/comdet/deep_audit.js";
  auditScript.dataset.comdetDeepAudit = "";
  auditScript.onload = function () {
    const guideScript = document.createElement("script");
    guideScript.src = "./js/comdet/undergrad_guides.js";
    guideScript.dataset.comdetUndergradGuides = "";
    guideScript.onload = function () {
      const walkerScript = document.createElement("script");
      walkerScript.src = "./js/comdet/execution_walkers.js";
      walkerScript.dataset.comdetExecutionWalkers = "";
      document.head.appendChild(walkerScript);
    };
